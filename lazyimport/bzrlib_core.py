...
Most commonly, the 'lazyimport' function is used to import other modules
in an on-demand fashion. Typically use looks like::

    from bzrlib.lazyimport import lazyimport
    lazyimport(globals(), '''
    from bzrlib import (
        errors,
        osutils,
        ...
        if real_obj is None:
            ...
            if obj is self:
                raise IllegalUseOfScopeReplacer(
                    name,
                    msg="Object tried to replace itself, check it's not using "
                        "its own scope.")
...
class ImportReplacer(ScopeReplacer):
    ...
    def _import(self, scope, name):
        ...
        if member is not None:
            module = __import__(
                module_python_path, scope, scope, [member], level=0)
...
class ImportProcessor(object):
    ...
    __slots__ = ['imports', '_lazyimport_class']

    def __init__(self, lazyimport_class=None):
        self.imports = {}
        if lazyimport_class is None:
            self._lazyimport_class = ImportReplacer
        else:
            self._lazyimport_class = lazyimport_class

    def lazyimport(self, scope, text):
    ...
    def _convert_imports(self, scope):
        ...
        try:
            for name, info in self.imports.items():
                self._lazyimport_class(
                    scope, name=name, module_path=info[0],
                    member=info[1], children=info[2])
        except AttributeError:  # we're in Python3
            for name, info in self.imports.items():
                self._lazyimport_class(
                    scope, name=name, module_path=info[0],
                    member=info[1], children=info[2])
...
def lazyimport(scope, text, lazyimport_class=None):
    ...
    return proc.lazyimport(scope, text)
